...
describe("Testing Image Controller Module", () => {
    ...
    describe("Testing ImageFinder class", () => {
        it("Image fjord should be found", () => {
            const readPath = path.join(__dirname ,mainPath, 'images', 'originalImages', 'fjord.jpg');
            expect(ImageFinder.find(readPath)).toBeTrue();
        });
        it("Image abc should not be found", () => {
            const readPath = path.join(__dirname ,mainPath, 'images', 'originalImages', 'abc.jpg');
            expect(ImageFinder.find(readPath)).toBeFalse();
        });
    });
    describe("Testing ImageResizer class", () => {
        const readPath = path.join(__dirname ,mainPath, 'images', 'originalImages', 'fjord.jpg');
        const writePath = path.join(__dirname ,mainPath, 'images', 'cachedImages', 'fjord_200_200.jpg');
    ...
    describe("Testing ImageBuilder class", () => {
        ...
        it("Image fjord_200_200 should be resized and created", async () => {
            expect(await ImageBuilder.build(readPath, writePath, "200", "200")).toBeTrue();
        });
        it("Image fjord_200_200 should not be created again", async () => {
            expect(await ImageBuilder.build(readPath, writePath, "200", "200")).toBeFalse();
        });
        afterAll(async () => {
            await fs.unlink(writePath);
        });
